...
typedef struct
{
  task_t *queue; // Fila de Tarefas Bloqueadas
...
typedef struct
{
  int max_msg; // Número máximo de mensagens
  int size_msg; // Tamanho de cada mensagem
  int count;    // Número de mensagens atual
  int status;   // Status da Fila 
  int head, tail; // Cabeça e Cauda da Fila
  semaphore_t sCons, sProd, sBuffer; // Semáforos para controle da Fila
  void *msg;   // Buffer para as mensagens
} mqueue_t ;
