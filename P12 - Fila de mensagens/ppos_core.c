...
int sem_destroy (semaphore_t *s)
{
    ...
    {
        return 0;
        printf("Tarefas liberadas !\n");
    }
    else
    {
        printf("Tarefas NÃO liberadas !\n");
        return -1;    
    }
}
...
int mqueue_init (mqueue_t *queue, int max, int size)
{
    ...
    if((queue->msg = (void *)malloc(max*size)) == NULL)
        return -1;

    sem_init(&queue->sBuffer, 1);
    sem_init(&queue->sCons, 0);
    sem_init(&queue->sProd, max);
...
int mqueue_send (mqueue_t *queue, void *msg)
{
    ...
    sem_down(&queue->sProd); // Solicita uma vaga para o produtor
    sem_down(&queue->sBuffer); // Solicita uso do buffer

    if(queue->status == 1)
    {
        // Copia a mensagem para o fim da fila
        memcpy((queue->msg + queue->tail * queue->size_msg), msg, queue->size_msg);
        // Atualiza o índice do final da fila
        queue->count++;
        queue->tail = (queue->tail + 1) % queue->max_msg;
    }

    sem_up(&queue->sBuffer); // Libera o buffer
    sem_up(&queue->sCons); // Libera uma vaga dos consumidores
...
int mqueue_recv (mqueue_t *queue, void *msg)
{
    ...
    sem_down(&queue->sCons); // Solicita uma vaga dos consumidores
    sem_down(&queue->sBuffer); // Solicita uso do buffer

    // Copia a mensagem do início da fila
    memcpy(msg,(queue->msg + queue->head * queue->size_msg), queue->size_msg);
    ...
    if(queue->count <= 0)
        ...
        queue->count--;
    queue->head = (queue->head + 1) % queue->max_msg;

    sem_up(&queue->sBuffer); // Libera o buffer
    sem_up(&queue->sProd); // Libera uma vaga para os produtores
...
int mqueue_destroy (mqueue_t *queue)
{
    ...
    if(queue->status == 1)
    {
        ...
        sem_destroy(&queue->sCons);
        sem_destroy(&queue->sProd);
        free(queue->msg);
        queue->status = 0;
        return 0;
    }
